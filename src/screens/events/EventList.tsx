...
interface EventListProps {
  type: 'pending' | 'assigned' | 'completed' | 'available' | 'scheduled' | 'past';
  title: string;
  onEventPress?: (event: Event) => void;
}

const EventList: React.FC<EventListProps> = ({ type, title, onEventPress }) => {
  ...
  const loadEvents = async () => {
    try {
      ...
      switch (type) {
        case 'pending':
          result = await executeRequest(() => eventService.getMyPendingEvents());
          break;
        case 'assigned':
          result = await executeRequest(() => eventService.getMyAssignedEvents());
          break;
        case 'completed':
          result = await executeRequest(() => eventService.getMyCompletedEvents());
          break;
        case 'available':
          result = await executeRequest(() => eventService.getAvailableRequests(filters));
          break;
        case 'scheduled':
          result = await executeRequest(() => eventService.getMyScheduledEvents());
          break;
        case 'past':
          result = await executeRequest(() => eventService.getMyPastPerformances());
          break;
        default:
          return;
      }
  ...
  const renderEventItem = ({ item }: { item: Event }) => (
    ...
    >
      ...
      {type === 'available' && item.status === 'pending_musician' && (
        <TouchableOpacity
